...
st.set_page_config(page_title="Calculadora de Filas", page_icon=":bar_chart:", layout="wide")
...
MODEL_CONFIG: Dict[str, Dict[str, Any]] = {
    ...
    "M/M/1/K": {
        ...
        "fields": [
            ...
            InputField("n", "n (probabilidade Pn)", field_type="int", required=False, placeholder="opcional"),
            InputField("t", "t (tempo)", field_type="float", required=False, placeholder="opcional"),
        ],
    },
    "M/M/S/K": {
...
def format_result_value(key: str, value: Any) -> Any:
    if isinstance(value, float):
        if key in TIME_KEYS:
            minutes = value * 60
            return f"{value:.5f} h ({minutes:.2f} min)"
...
def main() -> None:
    ...
    st.write("Selecione um modelo, forneca os parametros e visualize os principais indicadores.")
    ...
    selected_model = st.selectbox("Modelo", available_models, format_func=lambda key: key)
